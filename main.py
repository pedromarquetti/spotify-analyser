...
def print_stats(df: DataFrame):
    ...
    print(listen_time['artist'].value_counts().head(
        number_of_rows).to_string(header=False, index=True))

    min_by_artist = df.groupby('artist')['mins_played'].sum().astype(int)

    print('\nMinutes played per artist')
    print(min_by_artist.sort_values(ascending=False).head(
        number_of_rows).to_string(header=False, index=True))
    ...
    print(min_by_song.sort_values(by='mins_played', ascending=False).head(
        number_of_rows).to_string(header=False, index=True))


def fav_artist_by_year(df: DataFrame):
    yearly = df.groupby(['year', 'artist'], as_index=False)[
        'mins_played'].sum()
    yearly['rank'] = yearly.groupby(
        ['year'])['mins_played'].rank('dense', ascending=False)
    top = yearly[yearly['rank'] <= number_of_rows].reset_index(drop=True)

    color_map = {artist: plt.cm.viridis(
        i/len(top['artist'].unique())) for i, artist in enumerate(top['artist'].unique())}

    ax.set_xticks(top['year'])

    plt.title(
        f'Top {number_of_rows} artists since {top["year"].min()}', color='white')
    ...
    for year, data in top.groupby('year'):
        for i, value in data.sort_values(by='rank').iterrows():
            ...
            ax.scatter(year, rank, label=i,
                       color=color_map[value['artist']], marker='o')
...
def listen_time_by_year(df: DataFrame):
    ...
    yearly.plot(kind='bar', title='Minutes played by year',
                ylabel='Minutes played', xlabel='year', color='#1ED760')
...
def top_genre(df: DataFrame):
    ...
    track_id_groups = [track_id[i:i+50]
                       for i in range(0, len(track_id), 50)]  # group of 50 track IDs
...
def main(mode: str):
    ...
    match mode:
        ...
        case s:
            print(
                f'Unknown option {s}, available options are: \n{" ".join(supported_modes)}')
            print_stats(music_df)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Spotify JSON data analyser script')

    parser.add_argument(
        '-d', type=str, help='Specifies a folder containing unziped Spotify data (defaults to ./data/)')
    parser.add_argument(
        '-n', type=int, help='Specified number of rows to show')

    parser.add_argument('mode', nargs='?', default='stats',
                        help=f'supported modes: {" ".join(supported_modes)}, defaults to stats')

    parser.add_argument('artist', nargs='?', help='Artist to search')
